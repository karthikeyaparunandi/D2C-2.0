...
Model free DDP method with a simple double_pendulum experiment in MuJoCo simulator.
...
from params.double_pendulum_params import *
import os


class double_pendulum_D2C_DDP(DDP, ltv_sys_id_class):
	...
	def angle_normalize(self, x):
		'''
		Function to normalize the double_pendulum's angle from [0, Inf] to [-np.pi, np.pi]
		'''
...
if __name__=="__main__":
	...
	path_to_exp = "/experiments/double_pendulum/exp_1"
	MODEL_XML = path_to_model_free_DDP + "/models/inverted_double_pendulum.xml"
	path_to_file = path_to_model_free_DDP + path_to_exp + "/double_pendulum_policy.txt"
	training_cost_data_file = path_to_model_free_DDP + path_to_exp + "/training_cost_data.txt"
	path_to_data = path_to_model_free_DDP + path_to_exp + "/double_pendulum_D2C_DDP_data.txt"

	with open(path_to_data, 'w') as f:

		f.write("D2C training performed for an inverted double_pendulum task:\n\n")
	...
	initial_state = np.array([[0],[np.pi],[0],[0],[0],[0]])
	final_state = np.zeros((state_dimemsion,1))

	# Initiate the above class that contains objects specific to this problem
	D2C_double_pendulum = double_pendulum_D2C_DDP(initial_state, final_state, MODEL_XML, alpha, horizon, state_dimemsion, control_dimension, Q, Q_final, R)
	
	time_1 = time.time()

	# Run the DDP algorithm
	D2C_double_pendulum.iterate_ddp(n_iterations)
	...
	with open(training_cost_data_file, 'w') as f:
		...
		for cost in D2C_double_pendulum.episodic_cost_history:
		
			f.write("%s\n" % cost)

	# Save the obtained policy
	D2C_double_pendulum.save_policy(path_to_file)
	...
	print(D2C_double_pendulum.X_p[-1])
	
	# Plot the episodic cost during the training
	D2C_double_pendulum.plot_episodic_cost_history(save_to_path=path_to_model_free_DDP+path_to_exp+"/episodic_cost_training.png")
	
	# Test the obtained policy
	D2C_double_pendulum.test_episode(1, path_to_file)
