...
def perfcheck(nstart=0,nend=100,noisemax=100):
    ...
    #D2C
    i, x, y = np.loadtxt('fish_testing_data.txt', dtype=np.float64, delimiter=',\t',  unpack=True, usecols=(0,1,2))
    i = 100*i
    plt.figure(1)
    ...
    #DDPG
    DDPG_testing_data_location = "/media/karthikeya/Elements/DDPG_D2C/results/Fish/exp_1/"
    i, x, y, z = np.loadtxt(DDPG_testing_data_location + "data_1.txt", dtype=np.float64, delimiter='\t',  unpack=True, usecols=(0,1,2,3))
    i = 100*i
    print(x, y)
