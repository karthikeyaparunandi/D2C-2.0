...
if __name__=="__main__":
	...
	path_to_exp = path_to_model_free_DDP + "/experiments/swimmer/exp_10"

	path_to_file = path_to_exp + "/swimmer_policy.txt"
	training_cost_data_file = path_to_exp + "/training_cost_data.txt"
	path_to_data = path_to_exp + "/swimmer_D2C_DDP_data.txt"
	...
	if training_flag_on:
		...
		with open(path_to_data, 'w') as f:
			...
			f.write("D2C training performed for a swimmer motion planning task:\n\n")
		...
		swimmer.iterate_ddp(n_iterations, finite_difference_gradients_flag=True)
		
		LQR_feedback_on = False

		if LQR_feedback_on:
			swimmer.feedback(W_x_LQR, W_u_LQR, W_x_LQR_f)
		...
		swimmer.plot_episodic_cost_history(save_to_path=path_to_exp+"/episodic_cost_training.png")
	...
	if test_flag_on:
		...
		f = open(path_to_exp + "/swimmer_testing_data.txt", "a")
		...
		try:
			...
			for i in frange(0.5, 1.02, 0.02):
				...
				for j in range(n_samples):	
					...
					terminal_state = swimmer.test_episode(render=0, path=path_to_file, noise_stddev=i*u_max)
